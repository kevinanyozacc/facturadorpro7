...
export const editableRowItems = {
    ...
    computed: {
        showEditableItems() {
            return (
                this.configuration &&
                this.configuration.change_values_preview_document
            );
        },
        // canEditRowPrices()
        // {
    ...
    methods: {
        ...
        setRowCalculatedTotals(row, totals) {
            ...
            if (totals.total_value)
                row.total_value = _.round(totals.total_value, 2);
        ...
        getRowCalculatedUnitValue(affectation_igv_type_id, unit_price) {
            ...
            switch (affectation_igv_type_id) {
                case "10":
                    unit_value =
                        parseFloat(unit_price) / (1 + this.percentage_igv);
                    break;
        ...
        getRowCalculatedUnitPrice(affectation_igv_type_id, unit_value) {
            ...
            switch (affectation_igv_type_id) {
                case "10":
                    unit_price =
                        parseFloat(unit_value) * (1 + this.percentage_igv);
                    break;
        ...
        startRowTotalsVariables(row, input_total_value) {
            ...
            let total_igv = this.getRowCalculatedTotalIgv(
        ...
        setDifferentUnitPrice(row) {
            if (parseFloat(row.item.unit_price) != row.unit_price) {
                if (this.form.currency_type_id !== row.item.currency_type_id) {
                    if (
                        row.item.currency_type_id === "PEN" &&
                        this.form.currency_type_id === "USD"
                    ) {
                        row.item.unit_price = this.getFormatUnitPriceRow(
                            row.unit_price * this.form.exchange_rate_sale
                        );
                    } else {
                        row.item.unit_price = this.getFormatUnitPriceRow(
                            row.unit_price / this.form.exchange_rate_sale
        ...
        endRowCalculateTotal(
            ...
            this.setRowTotalsWithoutRounding(row, {
                total_value,
                total_base_igv,
                total_igv,
                total_taxes,
        ...
        recalculateRowTotals(row) {
            const {
                ...
                total,
                total_plastic_bag_taxes,
            } = this.startRowTotalsVariables(
                row,
                row.unit_value * row.quantity
            );

            this.setRowCalculatedTotals(row, {
                total_value,
                total_base_igv,
                total_igv,
                total_taxes,
        ...
        changeRowTotal(row) {
            ...
            if (row.input_has_plastic_bag_taxes)
                input_total -= row.total_plastic_bag_taxes;

            row.unit_price = parseFloat(input_total) / parseFloat(row.quantity);
            row.unit_value = this.getFormatUnitPriceRow(
                this.getRowCalculatedUnitValue(
                    row.affectation_igv_type_id,
                    row.unit_price
                )
            );
            if (row.item.purchase_unit_price > row.unit_price) {
                let decimal_input_unit_price_value = parseFloat(
                    row.input_unit_price_value
                );
                row.unit_price = decimal_input_unit_price_value;
                this.changeRowUnitPrice(row);
                return this.$message.error(
                    "El precio de compra no puede ser superior al precio de venta"
                );
            }
        ...
        changeRowTotalValue(row) {
            row.unit_value = this.getFormatUnitPriceRow(
                parseFloat(row.total_value) / parseFloat(row.quantity)
            );
            row.unit_price = this.getRowCalculatedUnitPrice(
                row.affectation_igv_type_id,
                row.unit_value
            );
            ...
            const {
                ...
                total,
                total_plastic_bag_taxes,
            } = this.startRowTotalsVariables(row, row.total_value);
