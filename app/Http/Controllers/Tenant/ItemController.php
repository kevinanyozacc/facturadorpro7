...
class ItemController extends Controller
{
    ...
    private function getOptimizedSearchRecords(Request $request)
    {
        ...
        //$perPage = config('tenant.items_per_page', 1);
        $sortField = $request->get('sort_field', 'id');
        $sortDirection = $request->get('sort_direction', 'desc');
        $perPage = 20; // 20 registros por página
        $maxRecords = 60; // Máximo 60 registros en total
       ...
       // Aplicar ordenamiento y paginación
        ...
        $total = min($totalQuery, $maxRecords); // Limitar a máximo 60 registros
        $offset = ($page - 1) * $perPage;
        
        // Si el offset supera el límite máximo, no hay registros para esta página
        if ($offset >= $maxRecords) {
            $records = collect([]);
        } else {
            $limitForQuery = min($perPage, $maxRecords - $offset);
