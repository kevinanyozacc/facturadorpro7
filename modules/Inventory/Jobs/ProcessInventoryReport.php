<?php
    ...
    class ProcessInventoryReport implements ShouldQueue
    {
        ...
        {
            Log::info("ProcessInventoryReport Start WebsiteId => " . $this->website_id . " TrayId => " . $this->tray_id);

            try {
                $website = Website::find($this->website_id);
                Log::info("Website encontrado: " . ($website ? 'SI' : 'NO'));
                
                $tenancy = app(Environment::class);
                $tenancy->tenant($website);
                Log::info("Tenancy configurado correctamente");
            } catch (Exception $e) {
                Log::error("Error configurando tenancy: " . $e->getMessage());
                throw $e;
            }

            $tray = DownloadTray::find($this->tray_id);
            Log::info("Tray encontrado: " . ($tray ? 'SI - Status: ' . $tray->status : 'NO'));
            $path = null;
            ...
            if (empty($tray)) {
                ...
                \Log::debug("No hay datos - website_id: " . var_export($website_id, true) . 
                ...
                try {
                    ...
                    if ($format === 'pdf') {
                        ...
                        Log::debug("Iniciando generación de Excel con " . count($records) . " registros");
                        $filename = 'INVENTORY_ReporteInv_' . date('YmdHis') . '-' . $tray->user_id;
                        ...
                        try {
                            ...
                            $inventoryExport
                                ...
                                ->format($format);
                            Log::debug("InventoryExport configurado correctamente");
                        } catch (Exception $e) {
                            Log::error("Error configurando InventoryExport: " . $e->getMessage());
                            throw $e;
                        }
                        ...
                        try {
                            ...
                            Log::debug("Excel almacenado correctamente");
                        } catch (Exception $e) {
                            Log::error("Error almacenando Excel: " . $e->getMessage());
                            throw $e;
                        }
                    ...
                    Log::error("ProcessInventoryReport Stack trace: " . $e->getTraceAsString());
                    
                    // Marcar como fallido en la bandeja
                    $tray->date_end = date('Y-m-d H:i:s');
                    $tray->status = 'FAILED';
                    $tray->save();
                }
                ...
                $query = ItemWarehouse::with(['warehouse', 'item' => function ($query) {
                    ...
                    ->whereHas('item', function ($q) {
                        $q->where([
                            ...
                            ->whereNotIsSet()
                            ->whereColumn('items.stock_min', '>=', 'item_warehouse.stock');
                    })->where('stock', '>', 0);
